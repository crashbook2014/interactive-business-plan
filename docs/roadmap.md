...
| **Deploy `analyze`** | Supabase access, to redeploy the edge function. The repo's `CR_SCHEMA` now asks the model for `obligations` and a `topic` on every finding; **production is still on version 6, which knows about neither**, so a real contract gets no obligations section until this is done. Nothing breaks in the meantime — the client renders a v6 response exactly as it did before, and `test/contract-review.test.js` asserts that against the literal v6 shape. | Obligations and dispute-ordering for pasted contracts |
